...
MASTER_TEMPLATES = {
    ...
    "刚需快餐小吃": {
        ...
        "target_keywords": ["面","皮","粉","饭","包","粥","小吃","麻辣烫","炸鸡","米线","凉皮","肉夹馍","饺子","馄饨","煎饼","便当","盖浇","砂锅","冒菜","卤味","鸭脖","鸡排","汉堡","披萨","拉面","拌面","酸辣粉","螺蛳粉","热干面","泡馍","擀面皮","锅贴","生煎","小笼","麻辣拌"],
...
BUSINESS_TYPE_TO_MASTER = {
    ...
    "大餐饮": "中餐正餐", "中餐": "中餐正餐",
    "火锅店": "火锅_烧烤", "烧烤店": "火锅_烧烤", "串串店": "火锅_烧烤", "涮锅店": "火锅_烧烤",
    ...
    # ═══════════ 14 个 master 业态名自映射（Phase 9E 修复）═══════════
    # 当 call site 直接传入 master 名（如 "低频目的零售"）作为 business_type 时，
    # 需要自映射才能通过 get_config() 拿到正确的 config + competitor_amap_types
    "异国_中高端正餐": "异国_中高端正餐",
    "火锅_烧烤": "火锅_烧烤",
    "刚需快餐小吃": "刚需快餐小吃",
    "中餐正餐": "中餐正餐",
...
INDUSTRY_RIGOR = {
    ...
    "火锅_烧烤": {
        "direct_competitor_rules": {
            ...
            "require_name_keyword_for_code": True,
            "substitute_before_direct": True,
            "name_keywords": ["火锅","烧烤","烤串","烤肉","涮肉","羊蝎子","烤鱼","小龙虾","大排档","串串香","串串","涮锅","小火锅","麻辣烫","冒菜"],
    ...
    "低频目的零售": {
        "direct_competitor_rules": {
            ...
            "strict_exclude_names": ["洗浴","足浴","会所","驾校","中介","房产","体检","医美","建材","五金","农贸","汽配","维修","彩票","美甲","快递","黄金回收"],
        },
        "substitute_competitor_rules": {
