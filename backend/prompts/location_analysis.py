...
4. 【数值防幻觉——POI 数量】报告正文中出现的所有 POI 数量（如"200米内X家""500米内X所""周边X个"），必须逐字照抄上方「周边 POI 数据」表格和「直接竞品/替代消费/客流锚点」字段中的数字。严禁：根据 POI 名称列表人工计数、根据城市经验自行估算、将相似品类数量混用、将 raw_count 写成精确数字。不确定时用区间表达（如"约15-20家"），禁止输出与数据源矛盾的精确数字。
5. 【绝对红线：禁止地名幻觉】绝不允许凭借自身知识库盲猜或捏造具体的 POI 名称（学校名、小区名、医院名、商场名等）！如果输入数据只有数量没有名称，只能用泛指代词（如"周边高校""周边社区""附近医疗机构"）。如果输入数据中有竞品清单或POI列表，则只能引用清单中真实存在的名称。凭空捏造输入数据中不存在的具体实体名称将被视为严重违规，报告全毁。
