...
def sim_full_chain(name, type_code, rigor_key, bt):
    ...
    if cat == "shopping":
        if any(kw in bt for kw in ("生鲜","水果","蔬菜","菜店","鲜果")) and is_real_fresh_retail(name):
            ...
            cat = "low_freq_retail"
        elif any(kw in bt for kw in ("日用百货","百货")) and is_real_convenience(name):
            cat = "convenience"
    dew_map = {
...
# Daily Goods direct: 购物服务 path (日用百货→convenience reclassification) + 060200 path (日杂/杂货/粮油/副食 in CONVENIENCE_KEEP)
for name in ["百货店","百货商店","日用百货","百货商行","百货铺","百货行"]:
