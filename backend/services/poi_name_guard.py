...
# 注意: "家" 不在此列表中（会误杀"家外家快捷酒店"），通过 _is_generic_candidate 单独处理
_NON_NAME_STARTS = re.compile(
    ...
    r'|个|座|栋|幢|所|条'  # 量词前缀，绝不会作为真实POI名称开头
    r')'
)

# 量词+POI后缀 组合模式 — 用于精准过滤"1个住宅小区"→"个住宅小区"的匹配片段
# 只匹配完整量词+后缀组合，避免误杀以量词字开头的真实POI（如"家外家"）
_QUANTIFIER_PLUS_GENERIC = re.compile(
    r'^(个|座|栋|幢|所|条|家)'
    r'(住宅小区|小区|住宅|办公建筑|写字楼|办公楼|教育机构|学校|小学|中学|高中|大学|学院|幼儿园|'
    r'医院|诊所|地铁站|公交线路|停车设施|停车场|流量节点|'
    r'商业体|便利店|超市|商场|酒店|宾馆|旅店|民宿|公寓|药房|卫生院|'
    r'新村|花园|家园|大厦|广场|购物商场|餐饮门店|餐厅|商铺|餐馆|药店|菜市场|门店|'
    r'竞品|同类店|文具店|店面|店铺)$'
)
...
_GENERIC_BLACKLIST = {
    ...
    "写字楼","高校",
    # 教育/培训类泛称 — 类别描述，非具体 POI 名称
    "培训学校","培训中心","托管中心","辅导中心","教育中心",
    "辅导学校","培训辅导","托管机构",
    # "现场" 词组 artifact — "场" 作为 POI 后缀被误匹配
    "需现场","不替代现场","天不同时段现场","现场",
    "替代现场","时段现场","同现场","核验现场",
    "如现场","建议现场","发现现场","析现场",
    # 描述性 artifact（含否定/替代 + 现场）
    "地现场","查现场","测现场",
    # 门店泛称 artifact（"门店"指代店铺，非具体 POI 名）
    "察同品牌门店","同品牌门店","品牌门店","察门店",
    "标客群和门店","客群和门店","和门店",
    "如目标客群与门店","目标客群与门店","目标客群是否与本店",
    "乘以门店","米内同品类门店","品类门店",
    # 兜底报告描述性 artifact
    "评分需结合近场","观察学校","不含大学",
    "沿街店","等茶饮店",
    "餐饮配套以社区","周边餐饮配套以","商业配套以零星",
    "放学时段蹲点记录",
    # 5ecf5e623833 批次
    "属同品牌加盟店","分流风险需现场","居民及学生到店",
    "米步行圈内购物商场","确认学校","家同类门店","观察门店",
    # f925a9612429 批次
    "孙大胜和同品牌店","家同品类店","公里学校",
    "公里社区","文具店","需重点验证学校",
    "取餐是否需要上楼",
...
_DESCRIPTIVE_MARKERS = (
    ...
    # 地点描述性前缀 — 不可能是 POI 名称
    "该位置", "此位置", "这个位置",
    # 距离描述 artifact — "X公里内/X米内" 后接任何词
    "公里内", "米内住", "米内同", "米内无", "米内商", "米内学",
    # 兜底报告 artifact
    "或远场", "内同类", "置为社区", "位置为",
    "已有分店", "当地已有", "无购物",
    # 描述/教育/分析 artifact
    ...
    # 数据来源
    "数据来", "口径说",
    # ★ 全业态报告通用 artifact — 不可能作为 POI 名称
    # 距离/半径描述片段（"X公里Y学校"中的"公里学校"）
    "公里学", "公里社", "公里商", "公里住", "公里无",
    "米内同", "米内住", "米内商", "米内学", "米内医",
    "米步行", "米范围", "米覆盖", "米半径",
    # 数量+品类泛称
    "家同类", "家同品", "个同类", "个同品", "家同品类",
    # 品牌关系描述
    "同品牌店", "同品牌加", "同品牌门", "品牌在当地",
    # 核验/操作描述
    "取餐是否", "验证学校", "验证门店", "重点验证",
    "放学时段", "蹲点记录", "现场核验", "实地走访",
    # 学生/客群描述
...
def _is_generic_candidate(candidate: str) -> bool:
    ...
    # ★ 量词+POI泛称组合: "个住宅小区" "栋办公建筑" "所教育机构" 等
    # 注意: "家" 也在此处理（精准匹配后缀，不误杀"家外家快捷酒店"）
    if _QUANTIFIER_PLUS_GENERIC.match(candidate):
        return True
