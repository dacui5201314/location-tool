...
- **v3.8.0** (2026-06-05) — 虚拟支付全链路闭环 + 小程序审核就绪：notify 回调六重校验 + 退款回退权益；confirm 端点兜底到账（appKey 验签，不依赖微信回调）；iOS 虚拟支付打通（正式环境 Key + Apple IAP）；用户充值记录页（自动过期 30min 未支付订单）；管理后台订单渠道标识 + 反馈删除功能；反馈上传适配小程序（单图上传替代多文件）；时区修复（UTC+8）；公交文案、data_quality_notes、AMap fallback 代码质量修复
