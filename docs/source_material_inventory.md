...
5. **已吸收闭环**：当前表内已蒸馏来源均已通过对应 Phase 吸收进 YAML，补全 `source_refs` 和业务回归测试。后续新增外部资料先做候选 source card（`derived_rule_only: true`），不改变报告行为；吸收到 YAML 前必须补 `source_refs` 和业务回归测试。
